# summarizer.py — Speed-Optimized Production Version
# ─────────────────────────────────────────────────────────────────────────────
# Short:    30–80w   | TF-IDF Extractive | 0 model calls | Instant
# Medium:   80–150w  | AI Abstractive    | ~8-12s  on CPU
# Detailed: 150–280w | AI + Extractive   | ~12-18s on CPU
#
# Key optimizations vs previous version:
#  - INPUT_WORD_LIMIT reduced (fewer tokens → faster attention)
#  - MAX_NEW_TOKENS reduced (stop model earlier)
#  - tokenizer max_length=200 (was 300 — wastes CPU on unused tokens)
#  - Robust tokenizer loading for mixed-up Drive folder tokenizer files
# ─────────────────────────────────────────────────────────────────────────────

import os, re, json, shutil, nltk, torch
...
LENGTH_SETTINGS = {
    ...
    "Detailed": {"min_length": 100, "max_length": 200},
}
OUTPUT_CAPS = {"Short": 80, "Medium": 150, "Detailed": 280}

# ── SPEED KEYS ────────────────────────────────────────────────────────────────
# INPUT_WORD_LIMIT: words fed to model
# Fewer words → fewer tokens → attention O(n²) much faster
# BART tokenizes ~1.3 BPE tokens/word on average
# 80 words ≈ 104 tokens, 100 words ≈ 130 tokens, 120 words ≈ 156 tokens
INPUT_WORD_LIMIT = {
    "Short":    80,    # not used (extractive only)
    "Medium":   90,    # ~117 tokens → 7-10s
    "Detailed": 110,   # ~143 tokens → 10-14s
}

# MAX_NEW_TOKENS: how many output tokens the model generates
# This is the single biggest driver of inference time
MAX_NEW_TOKENS = {
    "Short":    60,    # not used
    "Medium":   80,    # was 110 — 27% faster
    "Detailed": 130,   # was 190 — 32% faster
}

# tokenizer max_length: cap input token sequence
# Was 300 — model pads/truncates all sequences to this length → wasted cycles
TOKENIZER_MAX_LEN = 180   # safe margin above 143-token worst case
...
def _get_model_type(path):
    """Read config.json to find actual model_type (bart / t5 / etc.)"""
    try:
...
def _find_model_root(search_dir):
    """
    Find the directory with BOTH config.json AND model weights.
    Checks root first (preferred), then subfolders by path length.
    """
    ...
    if not candidates:
        return None
    candidates.sort(key=len)   # shortest path = closest to root
    return candidates[0]
...
def _load_tokenizer_robust(path, expected_type):
    """
    Load tokenizer correctly even when tokenizer_config.json lists the wrong class.

    Problem: Google Drive training folders mix tokenizer files — BART folder may have
    T5Tokenizer in tokenizer_config, T5 folder may have BartTokenizer. Direct
    from_pretrained() calls crash because they trust tokenizer_config.json blindly.

    Solution: read config.json to find actual model_type, then load the right
    tokenizer class directly. For T5, find spiece.model by walking subdirectories.
    """
    actual_type = _get_model_type(path)
    print(f"[INFO] Tokenizer load: path={path}, actual_type={actual_type}, expected={expected_type}")

    # ── T5: needs spiece.model ─────────────────────────────────────────────
    if actual_type == "t5" or expected_type == "t5":
        # Walk all subdirs to find spiece.model (may be in checkpoint subfolder)
        spiece_path = None
        for root, dirs, files in os.walk(path):
            if "spiece.model" in files:
                ...
                break
        if spiece_path:
            print(f"[INFO] Found spiece.model at {spiece_path}, loading T5Tokenizer directly")
            try:
                tok = T5Tokenizer(vocab_file=spiece_path, legacy=False)
                print("[INFO] T5Tokenizer loaded via spiece.model.")
                return tok
            except Exception as e:
                print(f"[WARN] T5Tokenizer(spiece) failed: {e}")

        # Try subfolder that has the T5 tokenizer_config
        for root, dirs, files in os.walk(path):
    ...
    try:
        ...
        print(f"[INFO] AutoTokenizer loaded: {type(tok).__name__}")
        return tok
    except Exception as e:
        raise RuntimeError(f"Could not load any tokenizer from {path}: {e}")
...
def load_bart():
    """Download (if needed) and load BART. Handles tokenizer_config mismatch."""
    try:
        ...
        import traceback
        print(f"[ERROR] BART load failed: {exc}"); traceback.print_exc()
        return None, None
...
def load_t5():
    ...
    Finds spiece.model by walking subdirectories — handles mixed tokenizer files
    in the Google Drive folder (BartTokenizer listed in tokenizer_config.json).
    """
    try:
        ...
        import traceback
        print(f"[ERROR] T5 load failed: {exc}"); traceback.print_exc()
        return None, None
...
def smart_trim(text, length_choice):
    ...
    limit = INPUT_WORD_LIMIT.get(length_choice, 100)
    words = text.split()
    if len(words) <= limit:
        return text
    try:
...
def _dedup(text):
    ...
    sents = _sent_tok(text); kept = []; ks = []
    for s in sents:
        s = s.strip(); w = set(s.lower().split())
        if not w: continue
        c = w - _STOP; dup = False
        for (ew, ec) in ks:
...
def extractive_summary(text, top_n=5, zone_based=False):
    try:
        ...
        print(f"[WARN] extractive_summary: {e}"); return ""


# ═════════════════════════════════════════════════════════════════════════════
# AI GENERATION  (speed-critical section)
# ═════════════════════════════════════════════════════════════════════════════
...
def _ai_generate(text, tokenizer, model, model_choice, min_len, max_len, length_choice):
    ...
    Speed optimizations:
    - max_length=TOKENIZER_MAX_LEN (was 300, now 180) — shorter sequences → faster
    - num_beams=1 (greedy decode — was 4 in some versions → 4× speedup)
    - MAX_NEW_TOKENS capped tightly per mode
    """
    ...
    try:
        enc = tokenizer(
            inp,
            return_tensors="pt",
            max_length=TOKENIZER_MAX_LEN,    # SPEED FIX: was 300
            truncation=True,
            padding=False,
        )
        ...
        with torch.no_grad():
            out = model.generate(
                ...
                num_beams=1,               # SPEED FIX: greedy decoding
                do_sample=False,
                early_stopping=True,
                min_length=safe_min,
                max_new_tokens=mnt,        # SPEED FIX: tight per-mode cap
        ...
        print(f"[ERROR] AI generation: {e}"); return ""
...
def _short_summary(cleaned, out_cap):
    """Zone-based 3-4 sentence Short summary — no model call needed."""
    try:
...
def generate_summary(input_text, tokenizer, model, model_choice, length_choice):
    ...
    tgt_min = int(wc * 0.45) if wc < 200 else {"Medium": 70, "Detailed": 130}[length_choice]
    ...
    if ai_wc >= tgt_min:
        ...
        extras = []; cur_wc = ai_wc if base_t else 0
        for x in (_sent_tok(ext) if ext else []):
            if cur_wc >= out_cap: break
