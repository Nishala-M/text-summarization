...
INPUT_WORD_LIMIT = {"Short": 100, "Medium": 120, "Detailed": 150}
MAX_NEW_TOKENS   = {"Short": 80,  "Medium": 110, "Detailed": 190}
...
def load_bart():
    try:
        _download_model_folder(BART_FOLDER_ID, BART_PATH, expected_model_type='bart')
...
def load_t5():
    try:
        _download_model_folder(T5_FOLDER_ID, T5_PATH, expected_model_type='t5')
        # Use AutoTokenizer to robustly handle tokenizer_config.json
        # that may say BartTokenizer (from mixed Drive folder files)
        # AutoTokenizer reads the actual vocab files present and picks the right class
        try:
...
def _ai_generate(text, tokenizer, model, model_choice, min_len, max_len, length_choice):
    ...
    try:
        enc = tokenizer(inp, return_tensors="pt", max_length=300,
