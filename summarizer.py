# summarizer.py — Fixed & Optimized Version
# ─────────────────────────────────────────────────────────────────────────────
# Short:    30–80w  | TF-IDF Extractive  | 0 model calls | Instant
# Medium:   80–150w | AI Abstractive     | 150w input    | ~5-8s
# Detailed: 150–280w| AI + Extractive    | 200w input    | ~8-12s
# ─────────────────────────────────────────────────────────────────────────────
...
def _download_and_extract(folder_id, output_path):
    """Download a Google Drive folder as zip and extract it."""
...
# ═════════════════════════════════════════════════════════════════════════════
# QUANTIZATION HELPER  (safe fallback)
# ═════════════════════════════════════════════════════════════════════════════

def _quantize(model):
    """INT8 quantization with graceful fallback to float32."""
    try:
        q = torch.quantization.quantize_dynamic(
            model, {torch.nn.Linear}, dtype=torch.qint8
        )
        print("[INFO] INT8 quantization applied — faster inference.")
        return q
    except Exception as e:
        print(f"[WARN] Quantization skipped ({e}), using float32.")
        return model


# ═════════════════════════════════════════════════════════════════════════════
# TOKENIZER HELPERS  — robust multi-strategy loading
# ═════════════════════════════════════════════════════════════════════════════

def _find_best_model_dir(base_path: str) -> str:
    """
    Walk base_path and return the directory that contains model.safetensors
    (or pytorch_model.bin).  Falls back to base_path itself.
    """
    for root, dirs, files in os.walk(base_path):
        if "model.safetensors" in files or "pytorch_model.bin" in files:
            print(f"[INFO] Using model source: {root}")
            return root
    return base_path


def _load_tokenizer_robust(path: str, preferred_cls, preferred_name: str):
    """
    Try loading a tokenizer with multiple strategies:
      1. preferred_cls.from_pretrained(path)          — exact class
      2. AutoTokenizer.from_pretrained(path)           — auto detect
      3. AutoTokenizer.from_pretrained(fallback_hub)   — HuggingFace Hub
    Returns tokenizer or None.
    """
    # Strategy 1 — preferred class
    try:
        tok = preferred_cls.from_pretrained(path)
        print(f"[INFO] {preferred_name} tokenizer loaded via {preferred_cls.__name__}.")
        return tok
    except Exception as e:
        print(f"[WARN] {preferred_cls.__name__} direct load failed ({e}), trying AutoTokenizer...")

    # Strategy 2 — AutoTokenizer on local path
    try:
        tok = AutoTokenizer.from_pretrained(path)
        print(f"[INFO] {preferred_name} tokenizer loaded via AutoTokenizer (local).")
        return tok
    except Exception as e:
        print(f"[WARN] AutoTokenizer local load failed ({e}), falling back to Hub...")

    # Strategy 3 — HuggingFace Hub fallback
    hub_map = {
        "BART": "facebook/bart-base",
        "T5":   "t5-small",
    }
    hub_id = hub_map.get(preferred_name)
    if hub_id:
        try:
            tok = AutoTokenizer.from_pretrained(hub_id)
            print(f"[INFO] {preferred_name} tokenizer loaded from Hub ({hub_id}).")
            return tok
        except Exception as e:
            print(f"[ERROR] Hub tokenizer load failed ({e}).")
...
def load_bart():
    """Download (if needed) and load BART tokenizer + model."""
    try:
        _download_and_extract(BART_FOLDER_ID, BART_PATH)

        model_dir = _find_best_model_dir(BART_PATH)
        print(f"[INFO] BART files: {os.listdir(model_dir)}")

        tok = _load_tokenizer_robust(model_dir, BartTokenizer, "BART")
        if tok is None:
            raise RuntimeError("All BART tokenizer strategies failed.")
...
def load_t5():
    """
    Download (if needed) and load T5 tokenizer + model.

    The saved T5 checkpoint uses a BPE/fast tokenizer (same family as BART's
    RoBERTa tokenizer), NOT the classic SentencePiece T5Tokenizer.
    We therefore use the same multi-strategy loader as BART.
    """
    try:
        _download_and_extract(T5_FOLDER_ID, T5_PATH)

        model_dir = _find_best_model_dir(T5_PATH)
        print(f"[INFO] T5 files: {os.listdir(model_dir)}")

        # The fine-tuned checkpoint may have a BPE tokenizer config
        # (merges.txt / vocab.json) — prefer AutoTokenizer, then T5Tokenizer
        tok = _load_tokenizer_robust(model_dir, AutoTokenizer, "T5")
        if tok is None:
            # Last resort: classic T5Tokenizer from Hub
            try:
                tok = T5Tokenizer.from_pretrained("t5-small", legacy=False)
                print("[INFO] T5 tokenizer loaded from Hub (t5-small) as last resort.")
            except Exception as e:
                raise RuntimeError(f"All T5 tokenizer strategies failed: {e}")
...
MAX_NEW_TOKENS   = {"Short": 100, "Medium": 200, "Detailed": 320}
    ...
    for i, w in enumerate(words):
        ...
        if title_count >= 3 and i + 1 < len(words):
            ...
            if nxt_clean and nxt_clean[0].isupper() and nxt_clean.lower() not in _TRIVIAL:
                prefix_words = [re.sub(r"[^a-z]", "", w2.lower()) for w2 in words[:i + 1]]
...
# ═════════════════════════════════════════════════════════════════════════════
# AI GENERATION  — optimized for speed
# ═════════════════════════════════════════════════════════════════════════════
...
def _ai_generate(text, tokenizer, model, model_choice,
    ...
    mnt      = MAX_NEW_TOKENS.get(length_choice, 200)
    ...
    try:
        ...
        with torch.no_grad():
            out = model.generate(
                ...
                # ── Speed tweaks ───────────────────────────────────────
                num_beams=2,            # was 4 — biggest single speed win
                do_sample=False,
                early_stopping=True,
                # ── Quality guards ─────────────────────────────────────
                min_length=safe_min,
                ...
                length_penalty=0.8,     # slightly shorter = faster decode
...
# ═════════════════════════════════════════════════════════════════════════════
# SHORT MODE  (pure extractive — instant)
# ═════════════════════════════════════════════════════════════════════════════
...
def generate_summary(input_text, tokenizer, model, model_choice, length_choice):
    ...
    # ── Short mode: extractive only, instant ─────────────────────────────────
    if length_choice == "Short":
        return _short_summary(cleaned, out_cap)

    # ── Medium / Detailed: AI abstractive ────────────────────────────────────
    ...
    final = ai_out if ai_out else ext
